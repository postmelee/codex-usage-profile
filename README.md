...
> [!IMPORTANT]
> The current production service supports the private preview, publish flow, and stable README card. A dedicated `/u/{handle}` share page and social preview are included in the next deployment candidate but are not yet live. The preview embed below will be replaced only after that deployment passes production smoke checks.

<!-- PROFILE_CARD_EMBED_PLACEHOLDER
Replace after the next production deployment and /u/{handle} smoke:
...
| Public profile | `/u/{handle}` | Next deployment | Human-readable share page |
