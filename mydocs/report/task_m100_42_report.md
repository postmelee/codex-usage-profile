...
승인된 결정에 따라 `@aws-sdk/client-s3`, contract v2, 최종 PNG SHA-256 digest revision/application ETag, `PROFILE_MEDIA_MODE=external` production runtime, `TEST_S3_*` gated integration, 기본 dry-run orphan cleanup을 구현했다. PR #48 리뷰 뒤 transient public read는 generic 503, stable conditional GET 경합은 1회 bounded retry로 보완했다. Share Studio UI와 실제 Cloud Run/R2 resource provisioning 등 제외 범위는 변경하지 않았다.

## 변경 파일 목록과 영향 범위

| 경로 | 변경 요약 | 영향 범위 |
|---|---|---|
| `src/profile-media/media-store-contract.js`, `src/profile-media/s3/` | contract v2, locale revision/stable key, S3-compatible adapter, invalid/unavailable 분류와 conditional read retry | 서버 media storage 경계 |
...
| `src/profile-backend/http.js`, `src/profile-backend/errors.js` | 기존 visibility endpoint orchestration, public media-only route, missing 404와 transient/submit retriable 503 | backend API와 stable image URL |
...
| 단계 완료 보고서 | 0개 | 6개 |
| 최종 통합 검증 | Task #42 기능 미구현 | 378 tests: 372 pass, 6 environment-gated skip, 0 fail |
...
| public missing/invalid와 provider 장애 분리 | OK — `not_found`/`invalid`/`conflict`는 동일 404, `unavailable`·예상 밖 adapter 오류는 generic 503과 `Retry-After: 5`를 반환한다. |
| stable HEAD→GET conditional race 복구 | OK — 첫 412는 최신 publication HEAD부터 재시도해 coherent body/ETag를 반환하고, 반복 412는 unavailable 503으로 수렴한다. |
...
| 전체 회귀·build·diff | OK — 372 pass, 6 skip, 0 fail; Vite production build 및 `git diff --check` 통과. |
...
- [Stage 6](../working/task_m100_42_stage6.md): public 404/503 오류 분리와 stable conditional read bounded retry를 구현·검증했다.
...
- 작업지시자는 2026-07-23 Stage 5 승인 뒤 최종 보고서 작성과 PR 게시 절차, PR #48 리뷰 발견 1·5의 Stage 6 보완을 승인했다.
