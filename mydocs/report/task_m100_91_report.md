...
- 대상 이슈: #91
- 마일스톤: M100
- 단계 수: 3 + PR·사용자 검토 보정 하위 단계 4
- 작업 목적: interactive login·submit 성공 결과를 표시하기 전에 local `gh` active account로 프로젝트를 star할지 `(Y/n)`으로 묻고 Enter를 기본 Yes로 처리하며, 부담을 줄인 안내 블록과 terminal link·color로 결과를 구분한다.
...
| `packages/codex-usage-profile-cli/src/output.js` | 지원 TTY에서 Profile·Card URL만 cyan OSC 8 hyperlink로 표시하고 README·JSON·projection을 평문으로 유지한다. | submit 결과 UX·automation 출력 |
...
| `packages/codex-usage-profile-cli/test/output.test.js` | Profile·Card exact OSC 8, README 평문과 JSON·non-TTY·color opt-out fallback을 검증한다. | submit output 표현·복사 계약 |
...
| `mydocs/plans/task_m100_91_impl.md` | 3개 Stage와 Stage 3.1~3.4 보정의 산출물·검증·완료 조건을 고정했다. | task 구현 기준 |
...
| `mydocs/working/task_m100_91_stage{N}.md` | `mydocs/working/` | `mydocs/working/task_m100_91_stage1.md`~`stage3.md`, `stage3_1.md`~`stage3_4.md` | OK | 제품 사용법이 아닌 단계별 구현·검증 증적만 내부 문서로 보관했다. |
...
| CLI package 전체 test | star prompt 회귀 기준 없음 | 69개 통과, 실패 0 |
| npm package exact entry | 13개 | 14개 (`src/github-star.js` 포함) |
| local package smoke | 신규 helper 미포함 | 6개 경계 통과 |
| branch 변경량(Stage 1~3.4 및 계획·보고 문서) | 0 | 22개 파일, +2,341/-8줄 |
...
| submit 성공 결과의 Profile·Card는 클릭 가능하되 README·automation 출력은 복사 가능한 평문이다. | OK — 지원 TTY에서 안전한 HTTP(S) Profile·Card만 cyan OSC 8이고 README exact line에는 escape가 없으며 JSON·non-TTY·`NO_COLOR`·`TERM=dumb` fallback을 검증했다. |
...
| 전체 repository 계약에 회귀가 없다. | OK — root test 749개 중 743개 통과, 환경 의존 6개 skip, 실패 0. |
| public release surface에 blocker가 없다. | OK — 2,468개 blob 검사, blocker 0, large blob skip 0. |
...
- 기존 PR #93의 owner review와 작업지시자의 terminal UX·first-run·submit link 검토를 반영했다. Stage 3.4 commit을 `publish/task91`에 게시하고 수용 기준과 CI를 다시 확인한다.
