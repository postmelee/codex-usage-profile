...
- 대상 이슈: #78
- 마일스톤: M100
- 단계 수: 6 (Stage 2와 Stage 5는 하위 단계로 분할해 실제 9개 단위)
...
| `src/profile-runtime/public-profile-resolver.js` | 신규. 단일 인덱스 읽기로 `cardLocale`, `handle`, `uploadedAt`만 조회 | D1 hot path |
...
| `src/profile-media/media-store-contract.js` | `social` 미디어 키와 선택적 capability | 세 스토어 어댑터 |
| `src/profile-media/publication-service.js` | 발행·설정 저장·refresh 경로에 소셜 이미지 편입 | 발행 파이프라인 |
| `src/profile-backend/http.js` | `GET\|HEAD /u/{handle}/social.png` | 공개 라우트 |
...
| 작업 문서 | `mydocs/plans`, `mydocs/working`, `mydocs/report` | 동일 | OK | 계획서 2개, 단계 보고서 8개, 최종 보고서 1개 |

제품 문서는 기존 `docs/readme-card.md` 갱신뿐이며 신규 공식 문서 루트 선택은 발생하지 않았다.
...
| 소셜 이미지는 handle당 하나, 설정 저장 시 같은 URL 갱신, unpublish 시 404 | OK — `cards/v2/public/{handle}/social.png` 단일 키. Stage 2.2 단위 테스트 |
...
```bash
npm test          # 679개 중 673 pass, 0 fail, 6 skipped
npm run test:e2e  # 64 pass, 0 fail (50.7s)
npx vite build    # 성공
...
- 계획 대비 편차 2건(`host-adapter.js` 수정, 소셜 이미지 2배 출력)에 대한 확인을 함께 요청한다.
