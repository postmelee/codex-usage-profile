...
기본 닫기 이징 `cubic-bezier(0.3, 0, 1, 1)`은 ease-in이라 느리게 출발해 빨라진다. 이동 모션에는 어색해서 인트로 모달만 감속 곡선 `cubic-bezier(0.05, 0.7, 0.1, 1)`을 쓴다. 빠르게 출발해 자기 자리에서 천천히 멎는다. 지속시간은 그대로 둔다.

이 값은 Share Studio와 공유하는 훅에 있으므로 `closeEasing` 옵션으로 분리했다. 기본값은 기존 곡선이라 Share Studio 동작은 바뀌지 않는다.
...
- OK. 인트로 닫기만 감속 곡선을 쓰고 훅 기본값은 그대로라 Share Studio는 영향받지 않는다
