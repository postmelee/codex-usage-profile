...
| `tests/profile-ui.spec.js` | Sites root-query device approval 진입점을 E2E로 고정했다. |
| Sites saved version 6 | source `b854c918916f9250ad4a69159232b127eae7b574`, 19-file production archive를 기존 Site에 저장했다. |
| Sites production deployment | saved version 6을 최종 environment revision 7로 private deployment해 `succeeded`를 확인했다. |
...
hosted smoke에서 발견한 device clean path redirect를 최소 source/test 보완으로 처리한 하위 단계 커밋은 `b854c91`이다.

## 본문 변경 정도 / 본문 무손실 여부

기존 제품·운영 문서는 변경하지 않았다. Account Usage Contract v1, GitHub OAuth/session, CLI token, publish/unpublish와 stable card URL 계약은 유지했다. source 변경은 Sites가 지원하는 root-query device approval 진입점에 한정했다.

Site 앞단의 extension 없는 clean HTML deep link 제약은 유지된다. device approval은 `/?view=device`로 복구했지만 `/u/{handle}` 직접 진입은 Worker보다 먼저 `307 /`로 이동한다. `.png` stable card와 JSON API는 이 제약을 받지 않는다.
...
- clean public HTML profile `/u/postmelee`는 `307 /`였다.
...
- `npm run verify:sites-production`: OK — artifact 4,653,963 bytes, client 7, migration 2, worker 2, expected bindings 3
...
- `/u/{handle}` clean public HTML profile은 Sites 앞단에서 `307 /`로 이동한다. stable card 공유만 MVP 계약으로 확정할지, Stage 6 전에 `/?profile={handle}` 같은 Sites 호환 public profile 진입점을 추가할지 결정이 필요하다.
...
- 현재 production candidate는 saved version 6, environment revision 7, access policy revision 13이다.
- Gate C 입력에는 stable-card-only MVP 범위 또는 Sites 호환 public HTML profile route 보완 중 하나를 명시해야 한다.
- public HTML profile을 기존 기능으로 유지해야 한다면 Gate C 전에 별도 root-query route source/test 보완과 새 saved version이 필요하다.
- final public access, README/공식 문서와 #43/#44/#45/#46·M100 metadata 변경은 Gate C exact 승인 뒤에만 수행한다.

## 승인 요청

- Stage 5 산출물과 owner-only 원복 결과를 검토해 승인해 달라.
- 권고안은 `/u/{handle}` 문제를 해결하거나 stable-card-only MVP 범위를 명시하기 전에는 Gate C 최종 공개 전환을 승인하지 않는 것이다.
